	...
	if($request == "getPlan"){
		...
		if ($plans) {
			foreach ($plans as $i => $p) {
					...
					$product = $p['theme']; //PARTSNAME
					$q = "
						SELECT COUNT(*) AS c
						FROM IRCS.T_PACKINGWK 
						WHERE PARTSNAME LIKE '".$product."%' AND IPADDRESS = '".$IRCS_IP."' 
		...
		$q = "SELECT COUNT(*) AS c
			FROM IRCS.T_PACKINGWK 
			WHERE PARTSNAME LIKE '".$parts."%' AND IPADDRESS = '".$IRCS_IP."' AND REGISTDATETIME >= TO_DATE('".$started."', 'yyyy-MM-dd HH24:MI:SS') AND PACKINGBOXCARDJUDGMENT = '1'";
		// $q = "
		// 	SELECT COUNT(*) AS c
		// 	FROM IRCS.T_PACKINGWK 
		...
		if(strtotime($_POST['time_start']) < strtotime('08:00:00')){
			$date_now = date('Y-m-d');
		...
		$status = "Pending";
		$sql = "INSERT INTO pcs_plan (Carmodel, Line, Target, Status, IRCS_Line, datetime_DB, takt_secs_DB, actual_start_DB, last_update_DB, IP_address,theme) VALUES ('".$car_maker."','".$line_name."','".$plan."','".$status."', '".$registlinename."', NOW(), ".$takt_secs.", '".$date_actual_start."', NOW(), '$IP_address','$parts')";
		if($db->execQuery($sql)){
